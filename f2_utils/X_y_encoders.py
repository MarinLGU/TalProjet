...
def encodeLemma(X):
    ftlemma1=[]
    ftlemma2=[]
    ft=fasttext.load_model('../w2cpretrained/fr.bin')
    for features in X:
...
vocs = [np.array(list(set(X_train2[:,i]))) for i in range(np.shape(X_train2)[1])]
...
if save:
    ...
    #     pickle.dump(Xenc1, f)
    #     f.close()
    with open('X_train2Enc.txt', 'wb') as f:
        pickle.dump(X_train2Enc, f)
        f.close()
    with open('X_test2Enc.txt', 'wb') as f:
        pickle.dump(X_test2Enc, f)
        f.close()
    with open('y_train2Enc.txt', 'wb') as f:
        pickle.dump(y_trainEnc, f)
        f.close()
    with open('y_test2Enc.txt', 'wb') as f:
